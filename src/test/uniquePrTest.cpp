...
UniquePtrTest::run(){
    ...
    auto pInv = makeInvestment(InvestmentType::STOCK, 1000);
    printf("size of pInv is %lu\n", sizeof(pInv));
    
    auto pInv2 = makeInvestment2(InvestmentType::STOCK, 1100);
    printf("size of pInv2 is %lu\n", sizeof(pInv2));

    // auto pInv3 = pInv2; // Error, std::unique_ptr can only be moved, not be copied
    auto pInv3 = std::move(pInv2);
    std::shared_ptr<Investment> pInvShared = makeInvestment(InvestmentType::BOND, 1200); 
